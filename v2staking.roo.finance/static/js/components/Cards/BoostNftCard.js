...
const BoostNftCard = ({
  ...
  useEffect(async () => {
    if (item.token_uri) {
      console.log('🚀 ~ useEffect ~ item.token_uri', item.token_uri);
      let image = await axios.get(
        `https://${item.token_uri.split('/')[4]}.ipfs.nftstorage.link/${
          item.token_uri.split('/')[5]
        }.json`
      );
      let cleanUri = image.data.image.replace(
        'ipfs://',
        'https://ipfs.io/ipfs/'
      );
